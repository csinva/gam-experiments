...
class LayerStack(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.layers = nn.ModuleList([layer_creator()
        ...
        for layer in self.layers[:n_layers]:
            ...
            if self.recompute_each_layer and x.requires_grad:
                x = checkpoint(partial(layer, **kwargs), x,
                               use_reentrant=False)  # type: ignore
            else:
                x = layer(x, **kwargs)
...
class PerFeatureTransformer(nn.Module):
    ...
    def __init__(  # noqa: C901, D417, PLR0913
        ...
        *,
        ...
        decoder_dict: dict[str,
            tuple[type[nn.Module] | None, int]] | None = None,
        ...
        **layer_kwargs: Any,
        ...
        def layer_creator(): return PerFeatureEncoderLayer(
            d_model=ninp,
            nhead=nhead,
        ...
        if repeat_same_layer:
            layer = layer_creator()
            def layer_creator(): return layer
    ...
    def _forward(  # noqa: PLR0912, C901
        ...
        *,
        ...
        if isinstance(x, dict):
            assert "main" in set(
                x.keys()), f"Main must be in input keys: {x.keys()}."
        else:
            x = {"main": x}
        ...
        if isinstance(y, dict):
            assert "main" in set(
                y.keys()), f"Main must be in input keys: {y.keys()}."
        else:
            y = {"main": y}
        ...
        test_encoder_out = encoder_out[:,
                                       single_eval_pos_:, -1].transpose(0, 1)
        ...
        *,
        ...
        if data_dags is not None:
            for b_i, data_dag in enumerate(data_dags):
                ...
                graph_pos_embs_targets = torch.zeros(
                    (1, k))  # shape: (num_targets, k)
                ...
                graph_pos_embs_targets -= graph_pos_embs_features.mean(
                    0, keepdim=True)
                graph_pos_embs_features -= graph_pos_embs_features.mean(
                    0, keepdim=True)
                ...
                x[b_i, :, :,
                    :k] += graph_pos_embs_features.to(y.device, y.dtype)

                graph_pos_embs_targets = (
                    graph_pos_embs_targets[None].expand(
                        seq_len, -1, -1).squeeze(-2)
                )
                y[b_i, :, :k] += graph_pos_embs_targets.to(y.device, y.dtype)
        else:
            assert not hasattr(
                self, "dag_pos_enc_dim") or not self.dag_pos_enc_dim
